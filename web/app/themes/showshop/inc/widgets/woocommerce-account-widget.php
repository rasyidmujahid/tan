...
class ASWCMyAccountWidget extends WP_Widget
{
function ASWCMyAccountWidget()
{
	$widget_ops = array('classname' => 'aswc-myaccount-widget', 'description' => __( 'WooCommerce My Account Widget shows order & account data', 'showshop' ) );
    $this->WP_Widget('ASWCMyAccountWidget', __( 'WooCommerce My Account', 'showshop' ), $widget_ops);
}
...
?>
	<p><label for="<?php echo $this->get_field_id('logged_out_title'); ?>"><?php _e('Logged out title:', 'showshop') ?></label>
		<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id('logged_out_title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('logged_out_title') ); ?>" value="<?php if (isset ( $instance['logged_out_title'])) echo esc_attr( $instance['logged_out_title'] ); else echo __('Customer Login', 'showshop'); ?>" /></p>
	
	<p><label for="<?php echo $this->get_field_id('logged_in_title'); ?>"><?php _e('Logged in title:', 'showshop') ?></label>
		<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id('logged_in_title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('logged_in_title') ); ?>" value="<?php if (isset ( $instance['logged_in_title'])) echo esc_attr( $instance['logged_in_title'] ); else echo __('Welcome %s', 'showshop'); ?>" /></p>
   	...
   	<p> <input type="checkbox" class="checkbox" id="<?php echo esc_attr( $this->get_field_id('show_cartlink') ); ?>" name="<?php echo esc_attr( $this->get_field_name('show_cartlink') ); ?>"<?php checked( $show_cartlink ); ?> />
		<label for="<?php echo $this->get_field_id('show_cartlink'); ?>"><?php _e( 'Show link to shopping cart', 'showshop' ); ?></label><br />
		...
		<label for="<?php echo $this->get_field_id('show_items'); ?>"><?php _e( 'Show number of items in cart', 'showshop' ); ?></label><br />
		...
		<label for="<?php echo $this->get_field_id('show_upload_new'); ?>"><?php _e( 'Show number of uploads left', 'showshop' ); ?></label><br />
		...
		<label for="<?php echo $this->get_field_id('show_upload'); ?>"><?php _e( 'Show number of uploads left', 'showshop' ); ?></label><br />
		<?php endif; ?>
		...
		<label for="<?php echo $this->get_field_id('show_unpaid'); ?>"><?php _e( 'Show number of unpaid orders', 'showshop' ); ?></label><br/>
		...
		<label for="<?php echo $this->get_field_id('show_pending'); ?>"><?php _e( 'Show number of uncompleted orders', 'showshop' ); ?></label><br>
		...
		<label for="<?php echo $this->get_field_id('show_logout_link'); ?>"><?php _e( 'Show logout link', 'showshop' ); ?></label>
	</p>
	<p><label for="<?php echo $this->get_field_id('wma_redirect'); ?>"><?php _e('Redirect to page after login:', 'showshop') ?></label>
		<select name="<?php echo esc_attr( $this->get_field_name('wma_redirect') ); ?>" class="widefat">
			<option value="">
				<?php echo esc_attr( __( 'Select page','showshop' ) ); ?></option> 
				<?php 
				$pages = get_pages(); 
	...
	<p><?php _e('Other options','showshop');?>:<br>
		...
		<label for="<?php echo $this->get_field_id('login_with_email'); ?>"><?php _e( 'Login with email address', 'showshop' ); ?></label>
	</p>
<?php
}
...
{	
	...
	$logged_out_title = apply_filters( 'widget_title', empty($instance['logged_out_title']) ? __('Customer Login', 'showshop') : $instance['logged_out_title'], $instance );
	
	$logged_in_title = apply_filters( 'widget_title', empty($instance['logged_in_title']) ? __('Welcome %s', 'showshop') : $instance['logged_in_title'], $instance );
	...
	if(is_multisite()) { $woo_ma_home=network_home_url(); } else {$woo_ma_home=home_url();}
		...
		if($c) {echo '<p><a class="woo-ma-button cart-link woo-ma-cart-link" href="'.get_permalink(wma_lang_id($cart_page_id)) .'" title="'. __('View your shopping cart','showshop').'">'.__('View your shopping cart','showshop').'</a></p>';}
		...
		echo '<ul class="clearfix woo-ma-list">';
			if($it) {
				...
				echo '<li class="woo-ma-link item">
						<a class="cart-contents-new" href="'.get_permalink(wma_lang_id($cart_page_id)).'" title="'. __('View your shopping cart', 'showshop').'">
							<span>'.$woocommerce->cart->cart_contents_count.'</span> '
							._n('product in your shopping cart','products in your shopping cart', $woocommerce->cart->cart_contents_count, 'showshop' ).'
						</a>
					</li>';
			} 
			if($u && function_exists('woocommerce_umf_admin_menu')) {
				...
				echo '<li class="woo-ma-link upload">
						<a href="'.get_permalink( $my_account_id ).'" title="'. __('Upload files', 'showshop').'">
							<span>'.$uploadfile.'</span> '
							._n('file to upload','files to upload', $uploadfile, 'showshop' ).'
						</a>
					</li>';
			}
            if($unew && class_exists('WPF_Uploads')) {
				...
				echo '<li class="woo-ma-link upload">
						<a href="'.get_permalink( $my_account_id ).'" title="'. __('Upload files', 'showshop').'">
							...
							._n('file to upload','files to upload', $uploadfile_new, 'showshop' ).'
						</a>
					</li>';
			}
			if($up) {
				echo '<li class="woo-ma-link paid">
						<a href="'.get_permalink( $my_account_id ).'" title="'. __('Pay orders', 'showshop').'">
							<span>'.$notpaid.'</span> '
							._n('payment required','payments required', $notpaid, 'showshop' ).'
						</a>
					</li>';
			}
			if($p) {
				echo '<li class="woo-ma-link pending">
						<a href="'.get_permalink( $my_account_id ).'" title="'. __('View uncompleted orders', 'showshop').'">
							<span>'.$notcompleted.'</span> '
							._n('order pending','orders pending', $notcompleted, 'showshop' ).'
						</a>
					</li>';
			} 
		echo '</ul>';
		echo '<p><a class="woo-ma-button woo-ma-myaccount-link myaccount-link" href="'.get_permalink( $my_account_id ).'" title="'. __('My Account','showshop').'">'.__('My Account','showshop').'</a></p>';
		if($lo==1) { echo '<p><a class="woo-ma-button woo-ma-logout-link logout-link" href="'.wp_logout_url($woo_ma_home).'" title="'. __('Log out','showshop').'">'.__('Log out','showshop').'</a></p>'; }
	}
	else {
		...
		if(isset($_GET['login']) && $_GET['login']=='failed') {
			...
			_e('Login failed, please try again','showshop');
			echo '</p>';
		}
		// login form
		$args = array(
			...
			'label_username' => __( 'Username','showshop'),
			'label_password' => __( 'Password','showshop'),
			'label_remember' => __( 'Remember Me','showshop' ),
			'label_log_in' => __( 'Log In','showshop'),
		...
		wp_login_form( $args );
		echo '<a class="woo-ma-link woo-ma-lost-pass" href="'. wp_lostpassword_url().'">'. __('Lost password?', 'showshop').'</a>';
		
		if(get_option('users_can_register')) {  
			echo ' <a class="woo-ma-button woo-ma-register-link register-link" href="'.get_permalink( get_option('woocommerce_myaccount_page_id') ).'" title="'. __('Register','showshop').'">'.__('Register','showshop').'</a>';
		}
		if($c) {
			echo '<p><a class="woo-ma-button woo-ma-cart-link cart-link" href="'.get_permalink(wma_lang_id($cart_page_id)) .'" title="'. __('View your shopping cart','showshop').'">'.__('View your shopping cart','showshop').'</a></p>';
		}
	}
	echo '</div>';
    echo $after_widget;
...
add_action( 'widgets_init', create_function('', 'return register_widget("ASWCMyAccountWidget");') );
	...
	<script type="text/javascript">
        ...
        jQuery('form#wma_login_form').submit(function(){
            ...
            if (jQuery(this).find('#user_login').val() == '' || jQuery(this).find('#user_pass').val() == '') {
              alert('<?php _e("Please fill in your username and password", "showshop"); ?>');
              return false;
            }
...
?>
	...
	if ( document.getElementById('wma_login_form') )
		document.getElementById('wma_login_form').childNodes[1].childNodes[1].childNodes[0].nodeValue = '<?php echo esc_js( __( 'Username or Email', 'showshop' ) ); ?>';
    </script>
