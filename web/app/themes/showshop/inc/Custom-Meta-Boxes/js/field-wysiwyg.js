AS.addCallbackForDeletedField( 'AS_wysiwyg', function( el ) {

	// Destroy WYSIWYG editors instances.
	el.find( '.as-wysiwyg textarea' ).each( function() {
...
AS.addCallbackForClonedField( 'AS_wysiwyg', function( newT ) {
	
	newT.find( '.as-wysiwyg' ).each( function (i) {
		...
		nameRegex = new RegExp( 'as-placeholder-name-' + fieldId, 'g' );
		idRegex   = new RegExp( 'as-placeholder-id-' + fieldId, 'g' );

		// Placeholder markup for the new wysiwyg is stored as a prop on var as_wysiwyg_editors
		// Copy, update ids & names & insert.
		el.html( as_wysiwyg_editors[fieldId].replace( nameRegex, name ).replace( idRegex, id ) );

		// If no settings for this field. Clone from placeholder.
		if ( typeof( tinyMCEPreInit.mceInit[ id ] ) === 'undefined' ) {
			var newSettings = jQuery.extend( {}, tinyMCEPreInit.mceInit[ 'as-placeholder-id-' + fieldId ] );
		...
		if ( typeof( tinyMCEPreInit.qtInit[ id ] ) === 'undefined' ) {
			var newQTS = jQuery.extend( {}, tinyMCEPreInit.qtInit[ 'as-placeholder-id-' + fieldId ] );
...
AS.addCallbackForSortStart( 'AS_wysiwyg', function( el ) {
...
AS.addCallbackForSortEnd( 'AS_wysiwyg', function( el ) {
