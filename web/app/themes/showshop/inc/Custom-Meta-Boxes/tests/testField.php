...
class FieldTestCase extends WP_UnitTestCase {
	...
	function testGetValues() {
		...
		// Single Value
		$field = new AS_Text_Field( 'foo', 'Title', array( 1 ) );
		$this->assertEquals( $field->get_values(), array( 1 ) );

		// Multiple Values - eg repeatable.
		$field = new AS_Text_Field( 'foo', 'Title', array( 1, 2 ), array( 'repeatable' => true ) );
	...
	function testIdAttrValue() {
		...
		$field = new AS_Text_Field( 'foo', 'Text', array( 1, 2 ) );

		// Standard use of ID attribute
		$id_attr = $field->get_the_id_attr();
		$this->assertEquals( $id_attr, 'foo-as-field-0' );

		// Using append
		$id_attr = $field->get_the_id_attr( 'bar' );
		$this->assertEquals( $id_attr, 'foo-as-field-0-bar' );

		// Repeatable 
		$field->field_index = 1;
		$id_attr = $field->get_the_id_attr();
		$this->assertEquals( $id_attr, 'foo-as-field-1' );
